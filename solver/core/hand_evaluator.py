...
优先使用 C++ 实现（约 20x 加速），如果不可用则回退到 Python 实现。
"""
from .card_utils import RANK_VALUES
...
_USE_CPP = False
try:
    # 统一使用绝对导入路径，防止双重加载导致的潜在冲突
    import solver.poker_solver_cpp as _cpp
    _USE_CPP = True
    print("[HandEvaluator] Using C++ acceleration (20x faster)")
except ImportError:
    # 尝试相对导入
    try:
        from . import poker_solver_cpp as _cpp
        _USE_CPP = True
        print("[HandEvaluator] Using C++ acceleration (via relative import)")
    except ImportError:
        print("[HandEvaluator] C++ extension not available, using Python fallback")
...
def calculate_equity(
    ...
    """
    log_debug("H7", "calculate_equity entry", "hand_evaluator.py:145", {"board": [str(c) for c in board]})
    ...
    if _USE_CPP:
        ...
        equity = _cpp.calculate_equity(
            hero_cards[0].rank, hero_cards[0].suit,
            hero_cards[1].rank, hero_cards[1].suit,
            villain_cards[0].rank, villain_cards[0].suit,
            villain_cards[1].rank, villain_cards[1].suit,
            board_cpp,
            num_simulations
        )
    ...
    # Python fallback
    return _calculate_equity_python(
        hero_cards, villain_cards, board, num_simulations, cache_key
    )


def _calculate_equity_python(
    hero_cards: list[Card],
    villain_cards: list[Card],
    board: list[Card],
    num_simulations: int,
